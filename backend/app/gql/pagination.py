...
def normalize_pagination(limit: int | None, offset: int | None):
    ...
    max_size = int(config.get("MAX_PAGE_SIZE"))
    default =  int(config.get("DEFAULT_PAGE_SIZE"))
    choices = tuple(config.get("PAGE_SIZE_CHOICES"))

    if limit is None:
        limit = default
    else:
        limit = max(1, min(int(limit), max_size))
        """
